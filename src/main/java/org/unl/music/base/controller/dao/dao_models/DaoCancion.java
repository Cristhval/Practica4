...
public class DaoCancion extends AdapterDao<Cancion> {
    ...
    private HashMap<String, String> toDict(Cancion arreglo) throws Exception {
        ...
        aux.put("albun", db.get(arreglo.getId_album()-1).getNombre());

        return aux;
    }

    public void shell_sort(Cancion[] arr, Integer type) {
        int n = arr.length;
        ...
        for (int gap = n / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < n; i++) {
                ...
                if (type == Utiles.ASCEDENTE) {
                    while (j >= gap && arr[j - gap].getNombre().toLowerCase()
                            .compareTo(key.getNombre().toLowerCase()) > 0) {
                        arr[j] = arr[j - gap];
                        j -= gap;
                    }
                } else {
    ...
    public LinkedList<HashMap<String, String>>  orderShell(Integer type) throws Exception {
        ...
        if (!listAll().isEmpty()) {
            ...
            System.out.println("Se ha demorado shell_sort " + endTime + " ms");
            lista.toList(arr);
        }
        return all(lista);
    }
...
//        System.out.println("Ordenamiento por shellsort con el nombre de Cancion ");
//        System.out.println(da.orderShell(1).print());
//        System.out.println("Ordenamiento por Quicksort con el nombre de Cancion");
//        System.out.println(da.orderName(1).print());
//    }
